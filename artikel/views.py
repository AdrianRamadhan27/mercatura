...
def create_artikel(request):
    ...
    if request.method == "POST":
        ...
        if post_form.is_valid():
            ...
            Post.objects.create(author=request.user, title=judul, description=deskripsi,
                                image_url=image_url, date=datetime.date.today())
            return HttpResponse()
...
def create_artikel_json(request):
    ...
    if request.method == "POST":
        ...
        if title is not None and title != "" and deskripsi is not None and deskripsi != "" and image_url is not None and image_url != "":
          post = Post.objects.create(author=request.user, title=title,
                                      description=deskripsi, image_url=image_url, date=datetime.date.today())
          ...
          response_data['judul'] = title
          response_data['deskripsi'] = deskripsi
          response_data['image_url'] = image_url
          response_data['id'] = post.id
          return JsonResponse({"status": True, "data": response_data}, status=200)
        return JsonResponse({
            "status": False,
